...
public class FurnitureSpawner : MonoBehaviour
{
    ...
    private void RemoveAnchorAutoScaling()
    {
        // we don't want the child game objects we spawn
        // to be scaled by OVRScene* components
        if (TryGetComponent(out OVRScenePlane plane))
        {
            plane.ScaleChildren = false;
            plane.OffsetChildren = false;
        }

        if (TryGetComponent(out OVRSceneVolume volume))
        {
            volume.ScaleChildren = false;
            volume.OffsetChildren = false;
        }
    }
}
